...
function paperPreview(row: typeof papers.$inferSelect) {
  return {
    id: row.id,
    title: row.title,
    authors: row.authors,
  ...
  getTopics() {
    const rows = metadataDb.all<{
      ...
      SELECT
        domain AS field,
        COUNT(*) AS papers,
        ROUND(AVG(quality_score), 1) AS avgScore,
  ...
  getTopicDetail(field: string) {
    ...
    return {
      ...
      representativePapers: rows
        .slice()
        .sort((a, b) => Number(b.qualityScore || 0) - Number(a.qualityScore || 0) || Number(b.year || 0) - Number(a.year || 0))
        .slice(0, 60)
        .map(paperPreview),
