  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5">
      ...
      <div className="ss-caveat compact">
        Paper Discussion = public, non-anonymous, visible by default, admin post-moderation.
        Mentor Review = verified anonymous, default pending, moderated, sample-threshold protected.
      </div>
      ...
      <div className="text-xs text-ink-muted">Showing page offset {offset}, page size {PAGE_SIZE}, status: {status}</div>
      ...
      <section className="space-y-3">
        <h2 className="font-semibold text-ink-text">Paper comments</h2>
        {!queue.comments.length && <p className="text-sm text-ink-muted">No comments on this page.</p>}
        {queue.comments.map((c: any) => (
          <ModerationCard
            ...
            meta={<><StatusBadge tone={getStatusTone(c.moderation_status || status)}>{getStatusLabel(c.moderation_status || status)}</StatusBadge><StatusBadge tone="amber">{c.comment_type}</StatusBadge><span>{c.nickname || `User #${c.user_id}`}</span><span>{c.created_at}</span></>}
            body={c.body}
            onDone={() => load(offset)}
          />
        ))}
      ...
      <section className="space-y-3">
        <h2 className="font-semibold text-ink-text">Mentor/group reviews</h2>
        {!queue.reviews.length && <p className="text-sm text-ink-muted">No mentor reviews on this page.</p>}
        {queue.reviews.map((r: any) => (
          <ModerationCard
            ...
            meta={<><StatusBadge tone={getStatusTone(r.moderation_status || status)}>{getStatusLabel(r.moderation_status || status)}</StatusBadge><StatusBadge tone="green">Verified anonymous</StatusBadge><span>{r.relationship_type || 'Other'}</span><span>{r.created_at}</span></>}
