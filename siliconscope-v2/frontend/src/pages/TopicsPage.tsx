    ...
    return (
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-sm text-ink-muted mb-2">
          {detail.papers} 篇论文 · 平均评分 {detail.avgScore} · 引用 {detail.citations}
        </p>
            ...
            <div className="space-y-1">
              {detail.byYear.map((y) => (
                <div key={y.year} className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-ink-secondary w-8 text-right">{y.count}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-1">
              {detail.byVenue.slice(0, 10).map((v) => (
                <div key={v.key} className="flex justify-between text-sm">
                  ...
                  <span className="text-ink-subtle">{v.count}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-1">
              {detail.authors.slice(0, 10).map((a) => (
                ...
                >
                  ...
                  <span className="text-ink-subtle">{a.papers} 篇</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-1">
              {detail.institutions.slice(0, 10).map((i) => (
                <div key={i.name} className="flex justify-between text-sm">
                  ...
                  <span className="text-ink-subtle">{i.papers} 篇</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            {detail.representativePapers.slice(0, 10).map((p) => (
              <div key={p.id} className="border-b border-line-subtle last:border-0 pb-2 last:pb-0">
                <div className="font-medium text-sm text-ink-text">{p.title}</div>
        ...
        {list.map((t) => (
          ...
          >
            <div className="flex justify-between items-start mb-2">
              ...
              <span className="text-xs text-ink-subtle">{t.papers} 篇</span>
            </div>
            <div className="flex gap-2 text-xs mb-2">
              <span className="px-2 py-0.5 rounded bg-brand-50 text-brand-700">S+ {t.sPlus}</span>
              <span className="px-2 py-0.5 rounded bg-surface-soft text-ink-secondary">S {t.s}</span>
              <span className="px-2 py-0.5 rounded bg-surface-soft text-ink-secondary">A {t.a}</span>
            </div>
